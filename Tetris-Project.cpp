...
node *tet;
void drawTextTimes(char* string,float x, float y) //Used to print a string in Times New Roman font
{
	glColor3f(0,1,1);
	int len, i;
	glRasterPos2f(x,y);
...
void drawTextHel(char* string,float x, float y)  //Used to print a string in Helvetica font
{
	glColor3f(0,0.4,0);
	int len, i;
	glRasterPos2f(x,y);
...
void rotate1(node *n){ //Implements rotation of the tetriminoes
	if(n -> type == 0){
		if(n -> rot == 0){
...
void init()
{
	...
	glLightModelfv(GL_LIGHT_MODEL_LOCAL_VIEWER, acolor);
}
...
void border(float x1, float y1, float z1, float w){ //Draws the border for Game Window with the color which is the same as the falling tetrimino
...
void borderexit(float x1, float y1, float z1, float w){ //Draws the border for Exit Screen Window with the color which is the same as the falling tetrimino
	glColor3f(1,1,0);
	glBegin(GL_QUADS);
        glVertex3f(x1, y1, z1);
        glVertex3f(x1 - 2*w + 0.02, y1, z1);
...
void keyboardexit(unsigned char key, int x, int y) //Used to exit the game when ESC key is pressed on the keyboard
{
	if(key == 27) {
		exit(0);
	}
...
void exitscreen() //Implements the exit screen after the game is ended in a new window by displaying score
{
...
int exitscreenn() //Creates the Exit Screen window
{
